...
# Created by https://github.com/sixtenbe from a fork of https://gist.github.com/endolith/250860
...
def usage():
  print "USAGE: peaks.py <user file path> <native file path>"
  sys.exit(0)
...
if __name__ == "__main__":
    ...
    data1, rate1 = processing.ffmpeg_load_audio(user)
    lengthUser = len(data1)

    data2, rate2 = processing.ffmpeg_load_audio(native)
    lengthNative = len(data2)



    i = 10000
    x = np.linspace(0,500,lengthUser)
    # y = (0.3*np.sin(x) + np.sin(1.3 * x) + 0.9 * np.sin(4.2 * x) + 0.06 *
    # np.random.randn(i))
    # y *= -1
    user = data1
    _maxuser, _minuser = peakdetect(user)

    native = data2
    _maxnative, _minnative = peakdetect(native)

    # _max, _min_peakdetect_parabole_fitter(y, x, 750, 0.30)
    xm = [p[0] for p in _maxuser]
    ym = [p[1] for p in _maxuser]
    xn = [p[0] for p in _maxnative]
    yn = [p[1] for p in _maxnative]
    # xn = [p[0] for p in _min]
    # yn = [p[1] for p in _min]

    output = [a+b for a, b in zip(_maxuser, _maxnative)]

    print output

    # pylab.plot(xm, ym, '.r-', label='Native')
    # pylab.plot(xn, yn, '.b-', label='Non-Native User')
